...
test.describe("observed private area", () => {
  ...
  test("account menu exposes only observed destinations", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name === "mobile", "Authenticated desktop menu evidence is the captured baseline");
    await page.goto("/minha-area");
