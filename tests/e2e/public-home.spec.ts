...
test.describe("public home observed contract", () => {
  ...
  test("exposes the observed hero and navigation actions", async ({ page }) => {
    ...
    await expect(page.locator('a[href="#beneficios"]').first()).toHaveAttribute("href", "#beneficios");
    await expect(page.locator('a[href="#como-funciona"]').first()).toHaveAttribute("href", "#como-funciona");
    await expect(page.locator('a[href="#planos"]').first()).toHaveAttribute("href", "#planos");
